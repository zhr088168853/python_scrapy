...
# 获取当前日期命名(2020-02-27-gz.csv)
n = time.strftime("%Y-%m-%d") + "-gd.csv"
fw = open(n, 'w', encoding='utf-8')
fw.write('province,confirm,suspect,dead,heal,new_confirm\n')
i = 0
while i<len(names):
    fw.write(names[i]+','+str(num1[i])+','+str(num2[i])+','+str(num3[i])+','+str(num4[i])+','+str(num5[i])+'\n')
    i = i + 1
else:
